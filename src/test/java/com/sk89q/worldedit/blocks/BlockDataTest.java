...
public class BlockDataTest {
	...
	public void testCycle() {
		...
		for (int type = 0; type < 256; ++type) {
			...
			if (type == BlockID.CLOTH || type == BlockID.STAINED_CLAY || type == BlockID.STAINED_GLASS || type == BlockID.STAINED_GLASS_PANE || type == BlockID.CARPET) {
				continue;
			}
	...
	private static void testCycle(final int increment) {
		...
		for (int type = 0; type < 256; ++type) {
			@SuppressWarnings("unchecked") final TreeSet<Integer> datas = (TreeSet<Integer>) datasTemplate.clone();
			while (!datas.isEmpty()) {
