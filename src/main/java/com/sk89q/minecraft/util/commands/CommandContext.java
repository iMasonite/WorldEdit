		...
		for (int nextArg = 0; nextArg < argList.size();) {
			...
			for (int i = 1; i < arg.length(); ++i) {
				...
				if (valueFlags.contains(flagName)) {
					if (this.valueFlags.containsKey(flagName)) throw new CommandException("Value flag '" + flagName + "' already given");
					
					if (nextArg >= argList.size()) throw new CommandException("No value specified for the '-" + flagName + "' flag.");
