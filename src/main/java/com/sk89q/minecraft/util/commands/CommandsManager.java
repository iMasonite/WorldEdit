...
/** <p>
 * Manager for handling commands. This allows you to easily process commands, including nested
 * commands, by correctly annotating methods of a class.
 * </p>
 * 
 * <p>
 * To use this, it is merely a matter of registering classes containing the commands (as methods
 * with the proper annotations) with the manager. When you want to process a command, use one of the
 * <code>execute</code> methods. If something is wrong, such as incorrect usage, insufficient
 * permissions, or a missing command altogether, an exception will be raised for upstream handling.
 * </p>
 * 
 * <p>
 * Methods of a class to be registered can be static, but if an injector is registered with the
 * class, the instances of the command classes will be created automatically and methods will be
 ...
 * 
 * <p>
 * This uses Java reflection extensively, but to reduce the overhead of reflection, command lookups
 * are completely cached on registration. This allows for fast command handling. Method invocation
 * still has to be done with reflection, but this is quite fast in that of itself.
 * </p>
 * 
 * @author sk89q
...
public abstract class CommandsManager<T> {
	 ...
	 * key of null, whereas child commands are cached under their respective {@link Method}. The child
	 * map has the key of the command name (one for each alias) with the method. */
	...
	/** Mapping of commands (not including aliases) with a description. This is only for top level
	 * commands. */
	 ...
	 * is specified, then the methods of the class will be registered to be called statically.
	 * Otherwise, new instances will be created of the command classes and methods will not be called
	 * statically. A List of {@link Command} annotations from registered commands is returned.
	 * 
	 * @param cls
	 * @return A List of {@link Command} annotations from registered commands, for use in eg. a
	...
	private List<Command> registerMethods(Class<?> cls, Method parent) {
		try {
			if (getInjector() == null) return registerMethods(cls, parent, null);
			else {
	...
	/** Checks to see whether there is a command named such at the root level. This will check aliases
	 * as well.
	 * 
	 * @param command
	 * @return */
	...
	public void executeMethod(Method parent, String[] args, T player, Object[] methodArgs, int level) throws CommandException {
		...
		if (method == null) {
			if (parent == null) throw new UnhandledCommandException();
			else throw new MissingNestedCommandException("Unknown command: " + cmdName, getNestedUsage(args, level - 1, parent, player));
		}
		...
		if (executeNested) {
			if (argsCount == 0) throw new MissingNestedCommandException("Sub-command required.", getNestedUsage(args, level, method, player));
			else {
			...
			if (context.argsLength() < cmd.min()) throw new CommandUsageException("Too few arguments.", getUsage(args, level, cmd));
			
			if (cmd.max() != -1 && context.argsLength() > cmd.max()) throw new CommandUsageException("Too many arguments.", getUsage(args, level, cmd));
			
			if (!cmd.anyFlags()) {
				for (char flag : context.getFlags()) {
					if (!newFlags.contains(flag)) throw new CommandUsageException("Unknown flag: " + flag, getUsage(args, level, cmd));
				}
			}
	...
	protected void checkPermission(T player, Method method) throws CommandException {
		if (!hasPermission(method, player)) throw new CommandPermissionsException();
	}
	...
	protected boolean hasPermission(Method method, T player) {
		...
		for (String perm : perms.value()) {
			if (hasPermission(player, perm)) return true;
		}
	...
	/** Get the injector used to create new instances. This can be null, in which case only classes
	 * will be registered statically. */
