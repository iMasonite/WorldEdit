// $Id$
/*
 * WorldEdit Copyright (C) 2011 sk89q <http://www.sk89q.com> and contributors This program is free
...
public class CommandRegistration {
	...
	public boolean register(List<CommandInfo> registered) {
		...
		if (registered == null || commandMap == null) return false;
		for (CommandInfo command : registered) {
			DynamicPluginCommand cmd = new DynamicPluginCommand(command.getAliases(), command.getDesc(), "/" + command.getAliases()[0] + " " + command.getUsage(), executor, command.getRegisteredWith(), plugin);
	...
	public CommandMap getCommandMap() {
		...
		if (commandMap == null) {
			if (fallbackCommands != null) {
				...
				Bukkit.getServer().getLogger().severe(plugin.getDescription().getName() + ": Could not retrieve server CommandMap, using fallback instead!");
	...
	public boolean unregisterCommands() {
		...
		if (knownCommands == null || aliases == null) return false;
