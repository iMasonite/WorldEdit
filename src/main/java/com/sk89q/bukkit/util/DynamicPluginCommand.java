...
/** An implementation of a dynamically registered {@link org.bukkit.command.Command} attached to a
 * plugin */
		...
		if (registeredWith instanceof CommandsManager<?>) {
			try {
				for (String permission : permissions) {
					if (((CommandsManager<CommandSender>) registeredWith).hasPermission(sender, permission)) return true;
				}
				return false;
			}
			...
			for (String permission : permissions) {
				if (PermissionsResolverManager.getInstance().hasPermission((OfflinePlayer) sender, permission)) return true;
			}
			return false;
		}
