...
public abstract class LocalPlayer {
		...
		while (y <= world.getMaxY() + 2) {
			...
			if (free == 2) {
				...
				if (spots == 2) {
					...
					if (type == BlockID.LAVA || type == BlockID.STATIONARY_LAVA) return false;
	...
	public boolean ascendToCeiling(int clearance, boolean alwaysGlass) {
		...
		// No free space above
		if (world.getBlockType(new Vector(x, y, z)) != 0) return false;
	...
	/** Get the point of the block being looked at. May return null. Will return the farthest away air
	 * block if useLastBlock is true and no other block is found.
	 * 
	 ...
	 * 
	 * @param yawOffset offset that is added to the player's yaw before determining the cardinal
	 *        direction
	 * 
	 * @return the direction */
	public PlayerDirection getCardinalDirection(int yawOffset) {
		if (getPitch() > 67.5) return PlayerDirection.DOWN;
		if (getPitch() < -67.5) return PlayerDirection.UP;
	...
	private static PlayerDirection getDirection(double rot) {
		if (0 <= rot && rot < 22.5) return PlayerDirection.SOUTH;
		else if (22.5 <= rot && rot < 67.5) return PlayerDirection.SOUTH_WEST;
		else if (67.5 <= rot && rot < 112.5) return PlayerDirection.WEST;
		else if (112.5 <= rot && rot < 157.5) return PlayerDirection.NORTH_WEST;
		else if (157.5 <= rot && rot < 202.5) return PlayerDirection.NORTH;
		else if (202.5 <= rot && rot < 247.5) return PlayerDirection.NORTH_EAST;
		else if (247.5 <= rot && rot < 292.5) return PlayerDirection.EAST;
		else if (292.5 <= rot && rot < 337.5) return PlayerDirection.SOUTH_EAST;
		else if (337.5 <= rot && rot < 360.0) return PlayerDirection.SOUTH;
		else return null;
	}
	...
	public BaseBlock getBlockInHand() throws WorldEditException {
		...
		if (!getWorld().isValidBlockType(typeId)) throw new NotABlockException(typeId);
	...
	public boolean equals(Object other) {
		if (!(other instanceof LocalPlayer)) return false;
	...
	public void checkPermission(String permission) throws WorldEditPermissionException {
		if (!hasPermission(permission)) throw new WorldEditPermissionException();
	}
