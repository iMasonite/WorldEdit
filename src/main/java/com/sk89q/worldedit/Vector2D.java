...
public class Vector2D {
	...
	public boolean containedWithinBlock(Vector2D min, Vector2D max) {
		return getBlockX() >= min.getBlockX() && getBlockX() <= max.getBlockX() && getBlockZ() >= min.getBlockZ() && getBlockZ() <= max.getBlockZ();
	}
	...
	public boolean isCollinearWith(Vector2D other) {
		if (x == 0 && z == 0) // this is a zero vector
		return true;
		
		final double otherX = other.x;
		final double otherZ = other.z;
		
		if (otherX == 0 && otherZ == 0) // other is a zero vector
		return true;
		...
		if (!Double.isNaN(quotientX)) return other.equals(multiply(quotientX));
		
		final double quotientZ = otherZ / z;
		if (!Double.isNaN(quotientZ)) return other.equals(multiply(quotientZ));
