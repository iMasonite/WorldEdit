 ...
 * 
 * Supported operators: Logical: &&, ||, ! (unary) Bitwise: ~ (unary), >>, << Arithmetic: +, -, *,
 * /, % (modulo), ^ (power), - (unary), --, ++ (prefix only) Comparison: <=, >=, >, <, ==, !=, ~=
 * (near)
 * 
 * Supported functions: abs, acos, asin, atan, atan2, cbrt, ceil, cos, cosh, exp, floor, ln, log,
 * log10, max, max, min, min, rint, round, sin, sinh, sqrt, tan, tanh and more. (See the Functions
 * class or the wiki)
 * 
 * Constants: e, pi
 * 
 * To compile an equation, run <code>Expression.compile("expression here", "var1", "var2"...)</code>
 * If you wish to run the equation multiple times, you can then optimize it, by calling
 * myExpression.optimize(); You can then run the equation as many times as you want by calling
 * myExpression.evaluate(var1, var2...) You do not need to pass values for all variables specified
 * while compiling. To query variables after evaluation, you can use
 * myExpression.getVariable("variable name"). To get a value out of these, use myVariable.getValue()
 * 
...
public class Expression {
	...
	private Expression(List<Token> tokens, String... variableNames) throws ExpressionException {
		...
		for (String variableName : variableNames) {
			if (variables.containsKey(variableName)) throw new ExpressionException(-1, "Tried to overwrite identifier '" + variableName + "'");
	...
	public double evaluate(double... values) throws EvaluationException {
		for (int i = 0; i < values.length; ++i) {
			...
			if (!(invokable instanceof Variable)) throw new EvaluationException(invokable.getPosition(), "Tried to assign constant " + variableName + ".");
