...
public class While extends Node {
	...
	public String toString() {
		if (footChecked) return "do { " + body + " } while (" + condition + ")";
		else return "while (" + condition + ") { " + body + " }";
	}
	...
	public RValue optimize() throws EvaluationException {
		...
		if (newCondition instanceof Constant && newCondition.getValue() <= 0) {
			...
			if (footChecked) // Foot-checked loops run at least once.
			return body.optimize();
			else // Loops that never run always return 0.0.
