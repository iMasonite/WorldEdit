...
public final class Functions {
	private static class Overload {
		...
		public Overload(Method method) throws IllegalArgumentException {
			...
			for (Class<?> parameter : parameters) {
				if (isSetter) throw new IllegalArgumentException("Method takes arguments that can't be cast to RValue.");
				...
				if (!RValue.class.isAssignableFrom(parameter)) throw new IllegalArgumentException("Method takes arguments that can't be cast to RValue.");
		...
		public boolean matches(boolean isSetter, RValue... args) {
			if (this.isSetter != isSetter) return false;
			
			if (this.method.getParameterTypes().length != args.length) return false;
	...
	private static Method getMethod(String name, boolean isSetter, RValue... args) throws NoSuchMethodException {
		...
		if (overloads != null) {
			for (Overload overload : overloads) {
				if (overload.matches(isSetter, args)) return overload.method;
			}
		}
	...
	public static double closest(RValue x, RValue y, RValue z, RValue index, RValue count, RValue stride) throws EvaluationException {
		return findClosest(Expression.getInstance().getFunctions().megabuf, x.getValue(), y.getValue(), z.getValue(), (int) index.getValue(), (int) count.getValue(), (int) stride.getValue());
	}
