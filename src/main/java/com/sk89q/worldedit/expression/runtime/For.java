...
public class For extends Node {
	...
	public double getValue() throws EvaluationException {
		...
		for (init.getValue(); condition.getValue() > 0; increment.getValue()) {
			if (iterations > 256) throw new EvaluationException(getPosition(), "Loop exceeded 256 iterations.");
			++iterations;
	...
	public RValue optimize() throws EvaluationException {
		...
		if (newCondition instanceof Constant && newCondition.getValue() <= 0) // If the condition is
																																					// always false, the loop
																																					// can be flattened.
		// So we run the init part and then return 0.0.
		return new Sequence(getPosition(), init, new Constant(getPosition(), 0.0)).optimize();
		
		// return new Sequence(getPosition(), init.optimize(), new While(getPosition(), condition, new
		// Sequence(getPosition(), body, increment), false)).optimize();
