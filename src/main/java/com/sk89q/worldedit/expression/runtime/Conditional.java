...
public class Conditional extends Node {
	...
	public double getValue() throws EvaluationException {
		if (condition.getValue() > 0.0) return truePart.getValue();
		else return falsePart == null ? 0.0 : falsePart.getValue();
	}
	...
	public String toString() {
		if (falsePart == null) return "if (" + condition + ") { " + truePart + " }";
		else if (truePart instanceof Sequence || falsePart instanceof Sequence) return "if (" + condition + ") { " + truePart + " } else { " + falsePart + " }";
		else return "(" + condition + ") ? (" + truePart + ") : (" + falsePart + ")";
	}
	...
	public RValue optimize() throws EvaluationException {
		...
		if (newCondition instanceof Constant) {
			if (newCondition.getValue() > 0) return truePart.optimize();
			else return falsePart == null ? new Constant(getPosition(), 0.0) : falsePart.optimize();
		}
