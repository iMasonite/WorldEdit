...
public class Parser {
	...
	private RValue parseStatements(boolean singleStatement) throws ParserException {
					...
					switch (keyword.charAt(0)) {
						...
						case 'f': { // for
							...
							if (peek().id() == ';') {
								...
								if (!(variableToken instanceof IdentifierToken)) throw new ParserException(variableToken.getPosition(), "Expected identifier");
								
								RValue variable = expression.getVariable(((IdentifierToken) variableToken).value, true);
								if (!(variable instanceof LValue)) throw new ParserException(variableToken.getPosition(), "Expected variable");
								++position;
								
								final Token equalsToken = peek();
								if (!(equalsToken instanceof OperatorToken) || !((OperatorToken) equalsToken).operator.equals("=")) throw new ParserException(variableToken.getPosition(), "Expected '=' or a term and ';'");
								++position;
						...
						case 's': // switch
							...
							consumeCharacter('{');
							while (peek().id() != '}') {
								if (position >= tokens.size()) throw new ParserException(current.getPosition(), "Expected '}' instead of EOF");
								if (defaultCase != null) throw new ParserException(current.getPosition(), "Expected '}' instead of " + peek());
								
								if (hasKeyword("case")) {
									++position;
									
									final Token valueToken = peek();
									if (!(valueToken instanceof NumberToken)) throw new ParserException(current.getPosition(), "Expected number instead of " + peek());
								...
								else throw new ParserException(current.getPosition(), "Expected 'case' or 'default' instead of " + peek());
							}
							consumeCharacter('}');
		...
		switch (statements.size()) {
			case 0:
				if (singleStatement) throw new ParserException(peek().getPosition(), "Statement expected.");
	...
	private final RValue parseExpression(boolean canBeEmpty) throws ParserException {
		...
		if (halfProcessed.isEmpty() && canBeEmpty) return new Sequence(peek().getPosition());
		
		return ParserProcessors.processExpression(halfProcessed);
	}
	
	private Token peek() {
		if (position >= tokens.size()) return new NullToken(tokens.get(tokens.size() - 1).getPosition() + 1);
	...
	private boolean hasKeyword(String keyword) {
		final Token next = peek();
		if (!(next instanceof KeywordToken)) return false;
		return ((KeywordToken) next).value.equals(keyword);
	}
	
	private void assertCharacter(char character) throws ParserException {
		final Token next = peek();
		if (next.id() != character) throw new ParserException(next.getPosition(), "Expected '" + character + "'");
	}
	
	private void assertKeyword(String keyword) throws ParserException {
		if (!hasKeyword(keyword)) throw new ParserException(peek().getPosition(), "Expected '" + keyword + "'");
	}
