...
public final class ParserProcessors {
	...
	static {
		...
		final Object[][][] binaryOpsLA = { { { "^", "pow" }, { "**", "pow" }, }, { { "*", "mul" }, { "/", "div" }, { "%", "mod" }, }, { { "+", "add" }, { "-", "sub" }, }, { { "<<", "shl" }, { ">>", "shr" }, }, { { "<", "lth" }, { ">", "gth" }, { "<=", "leq" }, { ">=", "geq" }, }, { { "==", "equ" }, { "!=", "neq" }, { "~=", "near" }, }, { { "&&", "and" }, }, { { "||", "or" }, }, };
		final Object[][][] binaryOpsRA = { { { "=", "ass" }, { "+=", "aadd" }, { "-=", "asub" }, { "*=", "amul" }, { "/=", "adiv" }, { "%=", "amod" }, { "^=", "aexp" }, }, };
		
		@SuppressWarnings("unchecked") final Map<String, String>[] lBinaryOpMapsLA = binaryOpMapsLA = new Map[binaryOpsLA.length];
		...
		@SuppressWarnings("unchecked") final Map<String, String>[] lBinaryOpMapsRA = binaryOpMapsRA = new Map[binaryOpsRA.length];
	...
	private static RValue processBinaryOpsLA(LinkedList<Identifiable> input, int level) throws ParserException {
		if (level < 0) return processUnaryOps(input);
	...
	private static RValue processBinaryOpsRA(LinkedList<Identifiable> input, int level) throws ParserException {
		if (level < 0) return processTernaryOps(input);
	...
	private static RValue processTernaryOps(LinkedList<Identifiable> input) throws ParserException {
		...
		for (Identifiable identifiable : input) {
			...
			if (conditionalsFound < 0) throw new ParserException(identifiable.getPosition(), "Unexpected ':'");
		...
		if (partsFound < 2) return processBinaryOpsLA(input, binaryOpMapsLA.length - 1);
	...
	private static RValue processUnaryOps(LinkedList<Identifiable> input) throws ParserException {
		...
		do {
			if (input.isEmpty()) throw new ParserException(-1, "Expression missing.");
		...
		if (!(center instanceof RValue)) throw new ParserException(center.getPosition(), "Expected expression, found " + center);
		...
		while (!input.isEmpty()) {
			...
			if (last instanceof Token) throw new ParserException(lastPosition, "Extra token found in expression: " + last);
			else if (last instanceof RValue) throw new ParserException(lastPosition, "Extra expression found: " + last);
			else throw new ParserException(lastPosition, "Extra element found: " + last);
		}
		return ret;
	}
}
