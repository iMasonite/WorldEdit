...
public interface CUIRegion {
	...
	/** Sends CUI events describing the region for versions of CUI equal to or greater than the value
	 * supplied by getProtocolVersion(). */
	public void describeCUI(LocalSession session, LocalPlayer player);
	
	/** Sends CUI events describing the region for versions of CUI smaller than the value supplied by
	 * getProtocolVersion(). */
	public void describeLegacyCUI(LocalSession session, LocalPlayer player);
	
	/** Returns the CUI version that is required to send up-to-date data. If the CUI version is smaller
	 * than this value, the legacy methods will be called.
	 * 
	 * @return */
