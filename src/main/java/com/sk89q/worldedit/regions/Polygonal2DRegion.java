...
public class Polygonal2DRegion extends AbstractRegion implements FlatRegion {
	...
	/** Recalculate the bounding box of this polygonal region. This should be called after points have
	 * been changed. */
	...
	public int getArea() {
		...
		for (i = 0; i < points.size(); ++i) {
			area += (points.get(j).getBlockX() + points.get(i).getBlockX()) * (points.get(j).getBlockZ() - points.get(i).getBlockZ());
			j = i;
		}
		
		return (int) Math.floor(Math.abs(area * 0.5) * (maxY - minY + 1));
	}
	...
	public void expand(Vector... changes) throws RegionOperationException {
		for (Vector change : changes) {
			if (change.getBlockX() != 0 || change.getBlockZ() != 0) throw new RegionOperationException("Polygons can only be expanded vertically.");
		}
	...
	public void contract(Vector... changes) throws RegionOperationException {
		for (Vector change : changes) {
			if (change.getBlockX() != 0 || change.getBlockZ() != 0) throw new RegionOperationException("Polygons can only be contracted vertically.");
		}
	...
	/** Returns string representation in the format "(x1, z1) - ... - (xN, zN) * (minY - maxY)"
	 * 
	 * @return string */
	@Override
	...
	public List<BlockVector2D> polygonize(int maxPoints) {
		if (maxPoints >= 0 && maxPoints < points.size()) throw new IllegalArgumentException("Cannot polygonize a this Polygonal2DRegion into the amount of points given.");
