...
public class Polygonal2DRegionSelector implements RegionSelector, CUIRegion {
	...
	public boolean selectPrimary(Vector pos) {
		if (pos.equals(pos1)) return false;
	...
	public boolean selectSecondary(Vector pos) {
		if (region.size() > 0) {
			...
			if (lastPoint.getBlockX() == pos.getBlockX() && lastPoint.getBlockZ() == pos.getBlockZ()) return false;
			
			if (maxPoints >= 0 && points.size() > maxPoints) return false;
		}
	...
	public BlockVector getPrimaryPosition() throws IncompleteRegionException {
		if (pos1 == null) throw new IncompleteRegionException();
