...
public class ConvexPolyhedralRegion extends AbstractRegion {
	...
	/** Accumulator for the barycenter of the polyhedron. Divide by vertices.size() to get the actual
	 * center. */
	private Vector centerAccum = Vector.ZERO;
	
	/** The last triangle that caused a {@link #contains(Vector)} to classify a point as "outside".
	...
	public boolean addVertex(Vector vertex) {
		...
		if (vertices.size() == 3) {
			if (vertexBacklog.contains(vertex)) return false;
			
			if (containsRaw(vertex)) return vertexBacklog.add(vertex);
		}
	...
	private boolean containsRaw(Vector pt) {
		if (lastTriangle != null && lastTriangle.above(pt)) return false;
