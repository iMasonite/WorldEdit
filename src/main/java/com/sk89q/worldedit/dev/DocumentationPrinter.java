...
public class DocumentationPrinter {
	...
	private static List<Class<?>> getCommandClasses(File dir) {
		...
		/*
		 * for (File f : dir.listFiles()) { if (!f.getName().matches("^.*\\.java$")) { continue; }
		 * String className = "com.sk89q.worldedit.commands." + f.getName().substring(0,
		 * f.getName().lastIndexOf(".")); Class<?> cls; try { cls = Class.forName(className, true,
		 * Thread.currentThread().getContextClassLoader()); } catch (ClassNotFoundException e) {
		 * continue; } classes.add(cls); }
		 */
