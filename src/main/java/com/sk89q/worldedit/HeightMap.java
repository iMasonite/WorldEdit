...
/** Allows applications of Kernels onto the region's heightmap. Currently only used for smoothing
		...
		for (int z = 0; z < height; ++z) {
			for (int x = 0; x < width; ++x) {
				...
				if (newHeight > curHeight) {
					...
					// Skip water/lava
					if (existing.getType() != BlockID.WATER && existing.getType() != BlockID.STATIONARY_WATER && existing.getType() != BlockID.LAVA && existing.getType() != BlockID.STATIONARY_LAVA) {
