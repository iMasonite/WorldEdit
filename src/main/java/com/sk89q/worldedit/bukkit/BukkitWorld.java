...
public class BukkitWorld extends LocalWorld {
		...
		try {
			...
			for (File f : nmsBlocksDir.listFiles()) {
				...
				if (NmsBlock.class.isAssignableFrom(testBlock)) {
					...
					nmsSetSafeMethod = nmsBlockType.getMethod("setSafely", BukkitWorld.class, Vector.class, com.sk89q.worldedit.foundation.Block.class, boolean.class);
					// phew
					break;
				}
			}
	...
	private class NmsBlockClassLoader extends ClassLoader {
		...
		public Class<?> loadClass(String name) throws ClassNotFoundException {
			if (!name.startsWith("CL-NMS")) return super.loadClass(name);
			else {
	...
	public boolean copyToWorld(Vector pt, BaseBlock block) {
		...
		if (block instanceof ContainerBlock) // Chests/dispenser
		return setContainerBlockContents(pt, ((ContainerBlock) block).getItems());
	...
	/** Gets the single block inventory for a potentially double chest. Handles people who have an old
	 * version of Bukkit. This should be replaced with
	 * {@link org.bukkit.block.Chest#getBlockInventory()} in a few months (now = March 2012) // note
	 * from future dev - lol
	 * 
	...
	private Inventory getBlockInventory(Chest chest) {
		try {
			...
			if (chest.getInventory() instanceof DoubleChestInventory) {
				...
				if (inven.getLeftSide().getHolder().equals(chest)) return inven.getLeftSide();
				else if (inven.getRightSide().getHolder().equals(chest)) return inven.getRightSide();
				else return inven;
			}
			else return chest.getInventory();
		}
	}
	...
	private static final EnumMap<TreeGenerator.TreeType, TreeType> treeTypeMapping = new EnumMap<TreeGenerator.TreeType, TreeType>(TreeGenerator.TreeType.class);
			...
			switch (type) {
				...
				case ARROWS:
					if (ent instanceof Projectile) {
						// covers: arrow, egg, enderpearl, fireball, fish, snowball, throwpotion,
						// thrownexpbottle
						ent.remove();
						++num;
					}
	...
	private BaseItemStack[] getContainerBlockContents(Vector pt) {
		Block block = world.getBlockAt(pt.getBlockX(), pt.getBlockY(), pt.getBlockZ());
		if (block == null) return new BaseItemStack[0];
		BlockState state = block.getState();
		if (!(state instanceof org.bukkit.inventory.InventoryHolder)) return new BaseItemStack[0];
	...
	public boolean equals(Object other) {
		if (!(other instanceof BukkitWorld)) return false;
