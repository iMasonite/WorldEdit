...
// $Id$
/*
 * This file is a part of WorldEdit. Copyright (c) sk89q <http://www.sk89q.com> Copyright (c) the
 * WorldEdit team and contributors This program is free software: you can redistribute it and/or
 * modify it under the terms of the GNU Lesser General Public License as published by the Free
 * Software (at your option) any later version. This program is distributed in the hope that it will
 * be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A GNU General Public License for more details. You should have received a copy of the
...
/** A blind handler of blocks with TileEntity data that directly access Minecraft's classes through
 * CraftBukkit. </p> Usage of this class may break terribly in the future, and therefore usage
 * should be trapped in a handler for {@link Throwable}. */
public class DefaultNmsBlock extends NmsBlock {
	
	private static final Logger logger = WorldEdit.logger;
	private static Field compoundMapField;
	private static final Field nmsBlock_isTileEntityField; // The field is deobfuscated but the method
																													// isn't. No idea why.
	...
	/** Create a new instance with a given type ID, data value, and previous {@link TileEntityBlock}
	 * -implementing object.
	 * 
	...
	/** Create a new instance with a given type ID, data value, and raw {@link NBTTagCompound} copy.
	 * 
	 * @param type block type ID
	 * @param data data value
	...
	public CompoundTag getNbtData() {
		if (nbtData == null) return new CompoundTag(getNbtId(), new HashMap<String, Tag>());
	...
	public static DefaultNmsBlock get(World world, Vector position, int type, int data) {
		if (!hasTileEntity(type)) return null;
	...
	public static boolean set(World world, Vector position, BaseBlock block) {
		...
		if (!hasTileEntity(world.getBlockTypeIdAt(position.getBlockX(), position.getBlockY(), position.getBlockZ()))) return false;
	...
	/** Tries to set a block 'safely', as in setting the block data to the location, and then
	 * triggering physics only at the end.
	 * 
	 * @param world world to set the block in
	 * @param position position to set the block at
	 * @param block the block to set
	...
	public static boolean hasTileEntity(int type) {
		...
		try {
			return nmsBlock_isTileEntityField.getBoolean(nmsBlock); // Once we have the field stord, gets
																															// are fast
		}
	...
	public static net.minecraft.server.v1_4_R1.Block getNmsBlock(int type) {
		if (type < 0 || type >= net.minecraft.server.v1_4_R1.Block.byId.length) return null;
		return net.minecraft.server.v1_4_R1.Block.byId[type];
	}
	
	/** Converts from a non-native NMS NBT structure to a native WorldEdit NBT structure.
	 * 
	...
	private static Tag toNative(NBTBase foreign) {
		...
		else if (foreign instanceof NBTTagByte) return new ByteTag(foreign.getName(), ((NBTTagByte) foreign).data);
		else if (foreign instanceof NBTTagByteArray) return new ByteArrayTag(foreign.getName(), ((NBTTagByteArray) foreign).data);
		else if (foreign instanceof NBTTagDouble) return new DoubleTag(foreign.getName(), ((NBTTagDouble) foreign).data);
		else if (foreign instanceof NBTTagFloat) return new FloatTag(foreign.getName(), ((NBTTagFloat) foreign).data);
		else if (foreign instanceof NBTTagInt) return new IntTag(foreign.getName(), ((NBTTagInt) foreign).data);
		else if (foreign instanceof NBTTagIntArray) return new IntArrayTag(foreign.getName(), ((NBTTagIntArray) foreign).data);
		...
		else if (foreign instanceof NBTTagLong) return new LongTag(foreign.getName(), ((NBTTagLong) foreign).data);
		else if (foreign instanceof NBTTagShort) return new ShortTag(foreign.getName(), ((NBTTagShort) foreign).data);
		else if (foreign instanceof NBTTagString) return new StringTag(foreign.getName(), ((NBTTagString) foreign).data);
		else if (foreign instanceof NBTTagEnd) return new EndTag();
		else throw new IllegalArgumentException("Don't know how to make native " + foreign.getClass().getCanonicalName());
	}
	...
	private static NBTBase fromNative(Tag foreign) {
		...
		else if (foreign instanceof ByteTag) return new NBTTagByte(foreign.getName(), ((ByteTag) foreign).getValue());
		else if (foreign instanceof ByteArrayTag) return new NBTTagByteArray(foreign.getName(), ((ByteArrayTag) foreign).getValue());
		else if (foreign instanceof DoubleTag) return new NBTTagDouble(foreign.getName(), ((DoubleTag) foreign).getValue());
		else if (foreign instanceof FloatTag) return new NBTTagFloat(foreign.getName(), ((FloatTag) foreign).getValue());
		else if (foreign instanceof IntTag) return new NBTTagInt(foreign.getName(), ((IntTag) foreign).getValue());
		else if (foreign instanceof IntArrayTag) return new NBTTagIntArray(foreign.getName(), ((IntArrayTag) foreign).getValue());
		...
		else if (foreign instanceof LongTag) return new NBTTagLong(foreign.getName(), ((LongTag) foreign).getValue());
		else if (foreign instanceof ShortTag) return new NBTTagShort(foreign.getName(), ((ShortTag) foreign).getValue());
		else if (foreign instanceof StringTag) return new NBTTagString(foreign.getName(), ((StringTag) foreign).getValue());
		else if (foreign instanceof EndTag) return new NBTTagEnd();
		else throw new IllegalArgumentException("Don't know how to make NMS " + foreign.getClass().getCanonicalName());
	}
