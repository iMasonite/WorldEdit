...
/** A pickaxe mode that recursively finds adjacent blocks within range of an initial block and of the
 * same type.
 * 
 * @author sk89q */
public class RecursivePickaxe implements BlockTool {
	...
	public boolean actPrimary(ServerInterface server, LocalConfiguration config, LocalPlayer player, LocalSession session, WorldVector clicked) {
		...
		if (initialType == BlockID.AIR) return true;
		
		if (initialType == BlockID.BEDROCK && !player.canDestroyBedrock()) return true;
		
		EditSession editSession = session.createEditSession(player);
		
		try {
			recurse(server, editSession, world, clicked.toBlockVector(), clicked, range, initialType, new HashSet<BlockVector>(), config.superPickaxeManyDrop);
		}
	 ...
	 * @param initialType
	 * @param visited */
	private static void recurse(ServerInterface server, EditSession editSession, LocalWorld world, BlockVector pos, Vector origin, double size, int initialType, Set<BlockVector> visited, boolean drop) throws MaxChangedBlocksException {
		
		final double distanceSq = origin.distanceSq(pos);
		if (distanceSq > size * size || visited.contains(pos)) return;
		
		visited.add(pos);
		
		if (editSession.getBlock(pos).getType() != initialType) return;
