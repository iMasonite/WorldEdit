...
/** Represents chunk stores that use Alpha's file format for storing chunks. The code to resolve the
 * filename is already implemented in this class and an inheriting class merely needs to implement
 * getInputStream().
 * 
 * @author sk89q */
public abstract class LegacyChunkStore extends ChunkStore {
	...
	public CompoundTag getChunkTag(Vector2D pos, LocalWorld world) throws DataException, IOException {
		...
		try {
			tag = nbt.readTag();
			if (!(tag instanceof CompoundTag)) throw new ChunkStoreException("CompoundTag expected for chunk; got " + tag.getClass().getName());
			...
			for (Map.Entry<String, Tag> entry : children.entrySet()) {
				if (entry.getKey().equals("Level")) {
					...
					else throw new ChunkStoreException("CompoundTag expected for 'Level'; got " + entry.getValue().getClass().getName());
				}
			}
			
			if (rootTag == null) throw new ChunkStoreException("Missing root 'Level' tag");
