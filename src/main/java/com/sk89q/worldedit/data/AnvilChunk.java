...
public class AnvilChunk implements Chunk {
	...
	public AnvilChunk(LocalWorld world, CompoundTag tag) throws DataException {
		...
		for (Tag rawSectionTag : sections) {
			...
			blocks[y] = NBTUtils.getChildTag(sectionTag.getValue(), "Blocks", ByteArrayTag.class).getValue();
			data[y] = NBTUtils.getChildTag(sectionTag.getValue(), "Data", ByteArrayTag.class).getValue();
			
			// 4096 ID block support
			if (sectionTag.getValue().containsKey("Add")) {
				blocksAdd[y] = NBTUtils.getChildTag(sectionTag.getValue(), "Add", ByteArrayTag.class).getValue();
			}
		}
		
		int sectionsize = 16 * 16 * 16;
		for (byte[] block : blocks) {
			if (block.length != sectionsize) throw new InvalidFormatException("Chunk blocks byte array expected " + "to be " + sectionsize + " bytes; found " + block.length);
		}
		
		for (byte[] element : data) {
			if (element.length != (sectionsize / 2)) throw new InvalidFormatException("Chunk block data byte array " + "expected to be " + sectionsize + " bytes; found " + element.length);
		}
	}
	...
	public int getBlockData(Vector pos) throws DataException {
		...
		try {
			if (!shift) return (data[section][index] & 0xF0) >> 4;
			else return data[section][index] & 0xF;
		}
		catch (IndexOutOfBoundsException e) {
			throw new DataException("Chunk does not contain position " + pos);
		}
	...
	private void populateTileEntities() throws DataException {
		...
		for (Tag tag : tags) {
			if (!(tag instanceof CompoundTag)) throw new InvalidFormatException("CompoundTag expected in TileEntities");
	...
	/** Get the map of tags keyed to strings for a block's tile entity data. May return null if there
	 * is no tile entity data. Not public yet because what this function returns isn't ideal for
	 * usage.
	 * 
	 * @param pos
	 * @return
	...
	public BaseBlock getBlock(Vector pos) throws DataException {
		...
		/*
		 * if (id == BlockID.WALL_SIGN || id == BlockID.SIGN_POST) { block = new SignBlock(id, data); }
		 * else if (id == BlockID.CHEST) { block = new ChestBlock(data); } else if (id ==
		 * BlockID.FURNACE || id == BlockID.BURNING_FURNACE) { block = new FurnaceBlock(id, data); }
		 * else if (id == BlockID.DISPENSER) { block = new DispenserBlock(data); } else if (id ==
		 * BlockID.MOB_SPAWNER) { block = new MobSpawnerBlock(data); } else if (id ==
		 * BlockID.NOTE_BLOCK) { block = new NoteBlock(data); } else {
		 */
