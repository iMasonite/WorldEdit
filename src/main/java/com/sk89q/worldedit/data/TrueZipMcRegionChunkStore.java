...
/** Represents the chunk store used by Minecraft but zipped. Uses the replacement classes for
 * java.util.zip.* from TrueZip.
 * 
 * @author sk89q */
public class TrueZipMcRegionChunkStore extends McRegionChunkStore {
	...
	/** Create an instance. The folder argument lets you choose a folder or path to look into in the
	 * ZIP for the files. Use a blank string for the folder to not look into a subdirectory.
	 * 
	 * @param zipFile
	 * @param folder
	 * @throws IOException
	...
	protected InputStream getInputStream(String name, String worldname) throws IOException, DataException {
		...
		if (folder != null) {
			...
			if (folder == null) throw new MissingWorldException("Target world is not present in ZIP.", worldname);
		}
		
		ZipEntry entry = getEntry(name);
		if (entry == null) throw new MissingChunkException();
		try {
	...
	public boolean isValid() {
		for (Enumeration<? extends ZipEntry> e = zip.entries(); e.hasMoreElements();) {
			...
			if (testEntry.getName().matches(".*\\.mcr$") || testEntry.getName().matches(".*\\.mca$")) return true;
		}
