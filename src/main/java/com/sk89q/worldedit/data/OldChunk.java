...
public class OldChunk implements Chunk {
	...
	public OldChunk(LocalWorld world, CompoundTag tag) throws DataException {
		...
		int size = 16 * 16 * 128;
		if (blocks.length != size) throw new InvalidFormatException("Chunk blocks byte array expected " + "to be " + size + " bytes; found " + blocks.length);
		
		if (data.length != (size / 2)) throw new InvalidFormatException("Chunk block data byte array " + "expected to be " + size + " bytes; found " + data.length);
	}
	...
	public int getBlockData(Vector pos) throws DataException {
		...
		try {
			if (!shift) return (data[index] & 0xF0) >> 4;
			else return data[index] & 0xF;
		}
		catch (IndexOutOfBoundsException e) {
			throw new DataException("Chunk does not contain position " + pos);
		}
