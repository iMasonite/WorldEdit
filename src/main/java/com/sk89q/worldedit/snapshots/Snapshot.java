...
public class Snapshot implements Comparable<Snapshot> {
	...
	public ChunkStore _getChunkStore() throws IOException, DataException {
		if (file.getName().toLowerCase().endsWith(".zip")) {
			try {
				...
				if (!chunkStore.isValid()) return new TrueZipLegacyChunkStore(file);
				...
				if (!chunkStore.isValid()) return new ZippedLegacyChunkStore(file);
				
				return chunkStore;
			}
		}
		else if (file.getName().toLowerCase().endsWith(".tar.bz2") || file.getName().toLowerCase().endsWith(".tar.gz") || file.getName().toLowerCase().endsWith(".tar")) {
			try {
				ChunkStore chunkStore = new TrueZipMcRegionChunkStore(file);
			...
			if (!chunkStore.isValid()) return new FileLegacyChunkStore(file);
	...
	public boolean containsWorld(String worldname) {
		try {
			...
			else return (file.getName().equalsIgnoreCase(worldname));
		}
		catch (IOException ex) {
			// Skip the file, but print an error
			logger.info("Could not load snapshot: " + file.getPath());
		}
	...
	public boolean equals(Object o) {
		if (o instanceof Snapshot) return file.equals(((Snapshot) o).file);
		return false;
	}
}
