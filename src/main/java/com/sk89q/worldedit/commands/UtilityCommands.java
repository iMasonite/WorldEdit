...
public class UtilityCommands {
	...
	@Command(aliases = { "/fixlava", "fixlava" },
			usage = "<radius>",
	...
	@Command(aliases = { "/fixwater", "fixwater" },
			usage = "<radius>",
	...
	@Command(aliases = { "/removeabove", "removeabove" },
			usage = "[size] [height]",
	...
	@Command(aliases = { "/removebelow", "removebelow" },
			usage = "[size] [height]",
	...
	@Command(aliases = { "/removenear", "removenear" },
			usage = "<block> [size]",
	...
	@Command(aliases = { "/replacenear", "replacenear" },
	...
	@Command(aliases = { "/ex", "/ext", "/extinguish", "ex", "ext", "extinguish" },
			usage = "[radius]",
	...
	public void butcher(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		// If you add flags here, please add them to com.sk89q.worldedit.tools.brushes.ButcherBrush as
		// well
	...
	@Command(aliases = { "remove", "rem", "rement" },
			usage = "<type> <radius>",
		...
		if (args.argsLength() == 0) {
			SortedSet<String> commands = new TreeSet<String>(new Comparator<String>() {
				...
				public int compare(String o1, String o2) {
					...
					if (ret == 0) return o1.compareToIgnoreCase(o2);
					return ret;
				}
			});
