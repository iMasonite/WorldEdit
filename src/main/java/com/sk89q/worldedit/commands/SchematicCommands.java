...
public class SchematicCommands {
	...
	public void save(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException, CommandException {
		...
		try {
			...
			if (parent != null && !parent.exists()) {
				if (!parent.mkdirs()) throw new CommandException("Could not create folder for schematics!");
			}
	...
	@Command(aliases = { "formats", "listformats", "f" },
	...
	public void list(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		if (files == null) throw new FilenameResolutionException(dir.getPath(), "Schematics directory invalid or not found.");
		...
		Arrays.sort(files, new Comparator<File>() {
			...
			public int compare(File f1, File f2) {
				...
				int result = sortType == 0 ? f1.getName().compareToIgnoreCase(f2.getName()) : // use name by
																																											// default
