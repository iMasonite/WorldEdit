...
public class SelectionCommands {
	...
	public void chunk(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		if (args.hasFlag('s')) {
			...
			if (args.argsLength() == 1) {
				// coords specified
				String[] coords = args.getString(0).split(",");
				if (coords.length != 2) throw new InsufficientArgumentsException("Invalid coordinates specified.");
	...
	@Command(aliases = { "toggleeditwand" },
			usage = "",
	...
	public void distr(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		if ((useData && distributionData.size() <= 0) || (!useData && distribution.size() <= 0)) {  // *Should*
																																																// always
																																																// be
																																																// false
		...
		if (useData) {
			for (Countable<BaseBlock> c : distributionData) {
				...
				String str = String.format("%-7s (%.3f%%) %s #%d:%d", String.valueOf(c.getAmount()), c.getAmount() / (double) size * 100, name == null ? "Unknown" : name, c.getID().getType(), c.getID().getData());
				player.print(str);
			}
		}
		else {
			for (Countable<Integer> c : distribution) {
				...
				String str = String.format("%-7s (%.3f%%) %s #%d", String.valueOf(c.getAmount()), c.getAmount() / (double) size * 100, block == null ? "Unknown" : block.getName(), c.getID());
				player.print(str);
			}
		}
	}
