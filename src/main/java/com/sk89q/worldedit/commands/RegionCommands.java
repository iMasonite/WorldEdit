...
public class RegionCommands {
	...
	@Command(aliases = { "/replace", "/re", "/rep" },
	...
	public void move(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		Vector dir = we.getDirection(player, args.argsLength() > 1 ? args.getString(1).toLowerCase() : "me");
		BaseBlock replace;
	...
	public void stack(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		int count = args.argsLength() > 0 ? Math.max(1, args.getInteger(0)) : 1;
		Vector dir = we.getDiagonalDirection(player, args.argsLength() > 1 ? args.getString(1).toLowerCase() : "me");
	...
	@Logging(REGION)
	public void forest(CommandContext args, LocalSession session, LocalPlayer player, EditSession editSession) throws WorldEditException {
		...
		int upperY = flatRegion.getMaximumY() + 1; // Increase by 1 to have trees generate above the
																								// selection 1 block
