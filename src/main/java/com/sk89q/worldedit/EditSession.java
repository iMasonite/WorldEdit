// $Id$
/*
 * WorldEditLibrary Copyright (C) 2010 sk89q <http://www.sk89q.com> and contributors This program is
...
/** This class can wrap all block editing operations into one "edit session" that stores the state of
 * the blocks before modification. This allows for easy undo or redo. In addition to that, this
 * class can use a "queue mode" that will know how to handle some special types of items such as
 * signs and torches. For example, torches must be placed only after there is already a block below
 * it, otherwise the torch will be placed as an item.
 * 
 * @author sk89q */
...
public class EditSession {
	...
	private DoubleArrayList<BlockVector, BaseBlock> original = new DoubleArrayList<BlockVector, BaseBlock>(true);
	
	/** Stores the current blocks. */
	private DoubleArrayList<BlockVector, BaseBlock> current = new DoubleArrayList<BlockVector, BaseBlock>(false);
	
	/** Blocks that should be placed before last. */
	private DoubleArrayList<BlockVector, BaseBlock> queueAfter = new DoubleArrayList<BlockVector, BaseBlock>(false);
	
	/** Blocks that should be placed last. */
	private DoubleArrayList<BlockVector, BaseBlock> queueLast = new DoubleArrayList<BlockVector, BaseBlock>(false);
	
	/** Blocks that should be placed after all other blocks. */
	private DoubleArrayList<BlockVector, BaseBlock> queueFinal = new DoubleArrayList<BlockVector, BaseBlock>(false);
	
	/** The maximum number of blocks to change at a time. If this number is exceeded, a
	 * MaxChangedBlocksException exception will be raised. -1 indicates no limit. */
	private int maxBlocks = -1;
	
	/** Indicates whether some types of blocks should be queued for best reproduction. */
	...
	public EditSession(LocalWorld world, int maxBlocks) {
		if (maxBlocks < -1) throw new IllegalArgumentException("Max blocks must be >= -1");
	...
	public boolean rawSetBlock(Vector pt, BaseBlock block) {
		...
		if (y < 0 || y > world.getMaxY()) return false;
		
		world.checkLoadedChunk(pt);
		
		// No invalid blocks
		if (!world.isValidBlockType(type)) return false;
	...
	/** Sets the block at position x, y, z with a block type. If queue mode is enabled, blocks may not
	 * be actually set in world until flushQueue() is called.
	 * 
	 * @param pt
	 * @param block
	...
	public boolean setBlock(Vector pt, BaseBlock block) throws MaxChangedBlocksException {
		...
		if (mask != null) {
			if (!mask.matches(this, blockPt)) return false;
		}
		
		// if (!original.containsKey(blockPt)) {
		original.put(blockPt, getBlock(pt));
		
		if (maxBlocks != -1 && original.size() > maxBlocks) throw new MaxChangedBlocksException(maxBlocks);
	...
	public boolean setBlockIfAir(Vector pt, BaseBlock block) throws MaxChangedBlocksException {
		if (!getBlock(pt).isAir()) return false;
		else return setBlock(pt, block);
	}
	...
	public BaseBlock getBlock(Vector pt) {
		...
		if (queued) {
			/*
			 * BlockVector blockPt = pt.toBlockVector(); if (current.containsKey(blockPt)) { return
			 * current.get(blockPt); }
			 */
		}
	...
	/** Get the maximum number of blocks that can be changed. -1 will be returned if disabled.
	 * 
	...
	public boolean setChanceBlockIfAir(Vector pos, BaseBlock block, double c) throws MaxChangedBlocksException {
		if (Math.random() <= c) return setBlockIfAir(pos, block);
		return false;
	}
	...
	public int getHighestTerrainBlock(int x, int z, int minY, int maxY, boolean naturalOnly) {
		for (int y = maxY; y >= minY; --y) {
			...
			if (naturalOnly ? BlockType.isNaturalTerrainBlock(id, data) : !BlockType.canPassThrough(id, data)) return y;
		}
		return minY;
	}
				...
				while (true) {
					...
					switch (type) {
						...
						case BlockID.ACTIVATOR_RAIL:
							...
							// They're also attached to the block they're ascending towards via
							// BlockType.getAttachment.
					...
					if (walked.contains(current)) {
						// Cycle detected => This will most likely go wrong, but there's nothing we can do about
						// it.
						break;
					}
				}
	...
	public int fillXZ(Vector origin, BaseBlock block, double radius, int depth, boolean recursive) throws MaxChangedBlocksException {
		...
		while (!queue.empty()) {
			...
			if (recursive) {
				...
				double dist = Math.sqrt(Math.pow(originX - cx, 2) + Math.pow(originZ - cz, 2));
	...
	public int makeWalls(final Region region, Pattern pattern) throws MaxChangedBlocksException {
		...
		final ArbitraryShape shape = new RegionShape(region) {
			...
			protected BaseBlock getMaterial(int x, int y, int z, BaseBlock defaultMaterial) {
				if (y > maxY || y < minY) // Put holes into the floor and ceiling by telling ArbitraryShape
																	// that the shape goes on
				// outside the region
		...
		for (int x = minX; x <= maxX; ++x) {
			for (int z = minZ; z <= maxZ; ++z) {
				for (int y = upperY; y >= lowerY; --y) {
					...
					if (y + 1 <= world.getMaxY() && !getBlock(new Vector(x, y, z)).isAir() && getBlock(above).isAir()) {
	...
	/** Turns the first 3 layers into dirt/grass and the bottom layers into rock, like a natural
	 * Minecraft mountain.
	 * 
	 * @param region
	 * @return number of blocks affected
		...
		for (int x = minX; x <= maxX; ++x) {
			for (int z = minZ; z <= maxZ; ++z) {
				...
				for (int y = upperY; y >= lowerY; --y) {
					...
					boolean isTransformable = blockType == BlockID.GRASS || blockType == BlockID.DIRT || blockType == BlockID.STONE;
	...
	public int drainArea(Vector pos, double radius) throws MaxChangedBlocksException {
		...
		while (!queue.empty()) {
			...
			// Check block type
			if (type != BlockID.WATER && type != BlockID.STATIONARY_WATER && type != BlockID.LAVA && type != BlockID.STATIONARY_LAVA) {
				continue;
			}
	 ...
	 * @return number of blocks affected
	 * @throws MaxChangedBlocksException */
	public int green(Vector pos, double radius, boolean onlyNormalDirt) throws MaxChangedBlocksException {
		int affected = 0;
	 ...
	 * @return number of trees created
	 * @throws MaxChangedBlocksException */
	public int makeForest(Iterable<Vector2D> it, int upperY, int lowerY, double density, TreeGenerator treeGenerator) throws MaxChangedBlocksException {
		if (upperY < lowerY) throw new IllegalArgumentException("upperY must be greater than or equal to lowerY");
	...
	public int makeShape(final Region region, final Vector zero, final Vector unit, final Pattern pattern, final String expressionString, final boolean hollow) throws ExpressionException, MaxChangedBlocksException {
		...
		final ArbitraryShape shape = new ArbitraryShape(region) {
			...
			protected BaseBlock getMaterial(int x, int y, int z, BaseBlock defaultMaterial) {
				...
				try {
					if (expression.evaluate(scaled.getX(), scaled.getY(), scaled.getZ(), defaultMaterial.getType(), defaultMaterial.getData()) <= 0) return null;
	...
	private static final Vector[] recurseDirections = { PlayerDirection.NORTH.vector(), PlayerDirection.EAST.vector(), PlayerDirection.SOUTH.vector(), PlayerDirection.WEST.vector(), PlayerDirection.UP.vector(), PlayerDirection.DOWN.vector(), };
	 ...
	 * @return number of blocks affected
	 * @throws MaxChangedBlocksException */
	public int drawSpline(Pattern pattern, List<Vector> nodevectors, double tension, double bias, double continuity, double quality, double radius, boolean filled) throws MaxChangedBlocksException {
	...
	private static Set<Vector> getHollowed(Set<Vector> vset) {
		...
		for (Vector v : vset) {
			...
			if (!(vset.contains(new Vector(x + 1, y, z)) && vset.contains(new Vector(x - 1, y, z)) && vset.contains(new Vector(x, y + 1, z)) && vset.contains(new Vector(x, y - 1, z)) && vset.contains(new Vector(x, y, z + 1)) && vset.contains(new Vector(x, y, z - 1)))) {
				returnset.add(v);
			}
		}
		return returnset;
	...
	public int makeBiomeShape(final Region region, final Vector zero, final Vector unit, final BiomeType biomeType, final String expressionString, final boolean hollow) throws ExpressionException, MaxChangedBlocksException {
		...
		final ArbitraryBiomeShape shape = new ArbitraryBiomeShape(region) {
			...
			protected BiomeType getBiome(int x, int z, BiomeType defaultBiomeType) {
				...
				try {
					if (expression.evaluate(scaled.getX(), scaled.getZ()) <= 0) return null;
