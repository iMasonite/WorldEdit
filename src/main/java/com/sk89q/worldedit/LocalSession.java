...
/** An instance of this represents the WorldEdit session of a user. A session stores history and
 * settings. Sessions are not tied particularly to any player and can be shuffled between players,
 * saved, and loaded.
 * 
 * @author sk89q */
public class LocalSession {
	...
	/** Remember an edit session for the undo history. If the history maximum size is reached, old edit
	 * sessions will be discarded.
	 * 
	 * @param editSession */
	...
	/** Get the region selector for defining the selection. If the selection was defined for a
	 * different world, the old selection will be discarded.
	 * 
	 * @param world
	 * @return position */
	...
	/** Get the region selector. This won't check worlds so make sure that this region selector isn't
	 * used blindly.
	 * 
	 * @return position */
	@Deprecated
	...
	public boolean isSelectionDefined(LocalWorld world) {
		if (selector.getIncompleteRegion().getWorld() == null || !selector.getIncompleteRegion().getWorld().equals(world)) return false;
		return selector.isDefined();
	}
	...
	/** Get the selection region. If you change the region, you should call learnRegionChanges(). If
	 * the selection is defined in a different world, the <code>IncompleteRegionException</code>
	...
	public Region getSelection(LocalWorld world) throws IncompleteRegionException {
		if (selector.getIncompleteRegion().getWorld() == null || !selector.getIncompleteRegion().getWorld().equals(world)) throw new IncompleteRegionException();
		return selector.getRegion();
	}
	...
	public CuboidClipboard getClipboard() throws EmptyClipboardException {
		if (clipboard == null) throw new EmptyClipboardException();
		return clipboard;
	}
	...
	public Vector getPlacementPosition(LocalPlayer player) throws IncompleteRegionException {
		if (!placeAtPos1) return player.getBlockIn();
	...
	/** Get the brush tool assigned to the item. If there is no tool assigned or the tool is not
	 * assigned, the slot will be replaced with the brush tool.
	 * 
	 * @param item
	 * @return the tool
	...
	public void setTool(int item, Tool tool) throws InvalidToolBindException {
		if (item > 0 && item < 255) throw new InvalidToolBindException(item, "Blocks can't be used");
		else if (item == config.wandItem) throw new InvalidToolBindException(item, "Already used for the wand");
		else if (item == config.navigationWand) throw new InvalidToolBindException(item, "Already used for the navigation wand");
	...
	public Calendar detectDate(String input) {
		...
		else return date.getBeginCalendar();
	}
