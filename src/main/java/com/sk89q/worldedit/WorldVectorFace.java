...
public class WorldVectorFace extends WorldVector {
	...
	public WorldVector getFaceVector() {
		return new WorldVector(getWorld(), getBlockX() - face.getModX(), getBlockY() - face.getModY(), getBlockZ() - face.getModZ());
	}
	
	/** Get a WorldVectorFace by comparing two vectors. Note that they need not be adjacent, as only
	 * the directions, not distance, will be taken into account.
	 * 
