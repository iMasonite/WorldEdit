...
public abstract class SchematicFormat {
	...
	public static SchematicFormat getFormat(File file) {
		...
		for (SchematicFormat format : SCHEMATIC_FORMATS.values()) {
			if (format.isOfFormat(file)) return format;
		}
		return null;
	}
	...
	public BaseBlock getBlockForId(int id, short data) {
		...
		switch (id) {
		/*
		 * case BlockID.WALL_SIGN: case BlockID.SIGN_POST: block = new SignBlock(id, data); break; case
		 * BlockID.CHEST: block = new ChestBlock(data); break; case BlockID.FURNACE: case
		 * BlockID.BURNING_FURNACE: block = new FurnaceBlock(id, data); break; case BlockID.DISPENSER:
		 * block = new DispenserBlock(data); break; case BlockID.MOB_SPAWNER: block = new
		 * MobSpawnerBlock(id); break; case BlockID.NOTE_BLOCK: block = new NoteBlock(data); break;
		 */
	...
	/** Saves the data from the specified CuboidClipboard to the given file, overwriting any existing
	 * data in the file
	 * 
