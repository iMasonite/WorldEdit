 ...
 * Supports {@link Node#tension tension}, {@link Node#bias bias} and {@link Node#continuity
 * continuity} parameters per {@link Node}
 * 
 * @author TomyLobo */
public class KochanekBartelsInterpolation implements Interpolation {
	...
	private void recalc() {
		...
		for (int i = 0; i < nNodes; ++i) {
			...
			final double ta = (1 - tensionA) * (1 + biasA) * (1 + continuityA) / 2; // Factor for lhs of
																																							// d[i]
			final double tb = (1 - tensionA) * (1 - biasA) * (1 - continuityA) / 2; // Factor for rhs of
																																							// d[i]
			final double tc = (1 - tensionB) * (1 + biasB) * (1 - continuityB) / 2; // Factor for lhs of
																																							// d[i+1]
			final double td = (1 - tensionB) * (1 - biasB) * (1 + continuityB) / 2; // Factor for rhs of
																																							// d[i+1]
	...
	private Vector retrieve(int index) {
		if (index < 0) return fastRetrieve(0);
		
		if (index >= nodes.size()) return fastRetrieve(nodes.size() - 1);
	...
	public Vector getPosition(double position) {
		if (coeffA == null) throw new IllegalStateException("Must call setNodes first.");
	...
	public double arcLength(double positionA, double positionB) {
		...
		if (positionA > positionB) return arcLength(positionB, positionA);
	...
	private double arcLengthRecursive(int indexLeft, double remainderLeft, int indexRight, double remainderRight) {
		switch (indexRight - indexLeft) {
			...
			case 1:
				...
				return arcLengthRecursive(indexLeft, remainderLeft, 1.0) + arcLengthRecursive(indexRight, 0.0, remainderRight);
				
			default:
				return arcLengthRecursive(indexLeft, remainderLeft, indexRight - 1, 1.0) + arcLengthRecursive(indexRight, 0.0, remainderRight);
		}
	}
	...
	public int getSegment(double position) {
		...
		if (position > 1) return Integer.MAX_VALUE;
