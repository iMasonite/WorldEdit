...
public class ReparametrisingInterpolation implements Interpolation {
	...
	private double arcToParameter(double arc) {
		if (cache.isEmpty()) throw new IllegalStateException("Must call setNodes first.");
		...
		if (leftArc == arc) return leftParameter;
		...
		if (rightArc == arc) return rightParameter;
	...
	private double evaluate(double arc, double leftArc, double leftParameter, double rightArc, double rightParameter) {
		...
		for (int i = 0; i < 10; ++i) {
			...
			if (midArc < leftArc) return leftParameter;
			
			if (midArc > rightArc) return rightParameter;
			
			if (Math.abs(midArc - arc) < 0.01) return midParameter;
