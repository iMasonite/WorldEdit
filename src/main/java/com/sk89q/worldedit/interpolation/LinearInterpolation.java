...
public class LinearInterpolation implements Interpolation {
	...
	public Vector getPosition(double position) {
		if (nodes == null) throw new IllegalStateException("Must call setNodes first.");
	...
	/*
	 * Formula for position: p1*(1-t) + p2*t Formula for position in Horner/monomial form: (p2-p1)*t +
	 * p1 1st Derivative: p2-p1 2nd Derivative: 0 Integral: (p2-p1)/2*t^2 + p1*t + constant Integral
	 * in Horner form: ((p2-p1)/2*t + p1)*t + constant
	 */
	...
	private double arcLengthRecursive(int indexA, double remainderA, int indexB, double remainderB) {
		switch (indexB - indexA) {
			...
			default:
				return arcLengthRecursive(indexA, remainderA, indexB - 1, 1.0) + arcLengthRecursive(indexB, 0.0, remainderB);
		}
	}
