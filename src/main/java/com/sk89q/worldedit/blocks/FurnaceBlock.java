...
public class FurnaceBlock extends ContainerBlock {
	...
	public void setNbtData(CompoundTag rootTag) throws DataException {
		...
		Tag t = values.get("id");
		if (!(t instanceof StringTag) || !((StringTag) t).getValue().equals("Furnace")) throw new DataException("'Furnace' tile entity expected");
		...
		for (Tag tag : items.getValue()) {
			if (!(tag instanceof CompoundTag)) throw new DataException("CompoundTag expected as child tag of Furnace Items");
