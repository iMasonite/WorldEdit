...
public class ChestBlock extends ContainerBlock {
	...
	public void setNbtData(CompoundTag rootTag) throws DataException {
		...
		Tag t = values.get("id");
		if (!(t instanceof StringTag) || !((StringTag) t).getValue().equals("Chest")) throw new DataException("'Chest' tile entity expected");
		
		List<CompoundTag> items = new ArrayList<CompoundTag>();
		
		for (Tag tag : NBTUtils.getChildTag(values, "Items", ListTag.class).getValue()) {
			if (!(tag instanceof CompoundTag)) throw new DataException("CompoundTag expected as child tag of Chest's Items");
