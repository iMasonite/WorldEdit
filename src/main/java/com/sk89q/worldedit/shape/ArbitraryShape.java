...
/** Generates solid and hollow shapes according to materials returned by the {@link #getMaterial}
 * method.
 * 
 * @author TomyLobo */
public abstract class ArbitraryShape {
	...
	/** Cache entries: 0 = unknown -1 = outside -2 = inside but type and data 0 > 0 = inside, value =
	 * (type | (data << 8)), not handling data < 0 */
	...
	private boolean isInsideCached(int x, int y, int z, Pattern pattern) {
		...
		switch (cache[index]) {
			case 0:
				// unknown block, meaning they must be outside the extent at this stage, but might still be
				// inside the shape
