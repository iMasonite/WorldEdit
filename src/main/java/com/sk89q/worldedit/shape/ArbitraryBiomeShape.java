...
/** Generates solid and hollow shapes according to materials returned by the {@link #getBiome}
 * method.
 * 
 * @author TomyLobo */
public abstract class ArbitraryBiomeShape {
	...
	/** Cache entries: null = unknown OUTSIDE = outside else = inside */
	...
	private boolean isInsideCached(int x, int z, BiomeType biomeType) {
		...
		final BiomeType cacheEntry = cache[index];
		if (cacheEntry == null) // unknown block, meaning they must be outside the extent at this stage,
														// but might still be inside the shape
