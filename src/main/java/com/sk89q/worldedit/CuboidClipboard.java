...
public class CuboidClipboard {
	...
	public void rotate2D(int angle) {
		...
		final BaseBlock newData[][][] = new BaseBlock[Math.abs(sizeRotated.getBlockX())][Math.abs(sizeRotated.getBlockY())][Math.abs(sizeRotated.getBlockZ())];
		...
		data = newData;
		size = new Vector(Math.abs(sizeRotated.getBlockX()), Math.abs(sizeRotated.getBlockY()), Math.abs(sizeRotated.getBlockZ()));
	 ...
	 * 
	 * @param The point, relative to the origin of the copy (0, 0, 0) and not to the actual copy
	 *        origin.
	 * @return air, if this block was outside the (non-cuboid) selection while copying
	 * @throws ArrayIndexOutOfBoundsException if the position is outside the bounds of the
	 *         CuboidClipboard
	...
	public BaseBlock getPoint(Vector pos) throws ArrayIndexOutOfBoundsException {
		...
		if (block == null) return new BaseBlock(BlockID.AIR);
