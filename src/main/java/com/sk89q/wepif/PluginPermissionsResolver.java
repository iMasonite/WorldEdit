...
public class PluginPermissionsResolver implements PermissionsResolver {
	...
	public static PermissionsResolver factory(Server server, YAMLProcessor config) {
		...
		if (serviceProvider != null) return new PluginPermissionsResolver(serviceProvider.getProvider(), serviceProvider.getPlugin());
		
		// Looking for plugin
		for (Plugin plugin : server.getPluginManager().getPlugins()) {
			if (plugin instanceof PermissionsProvider) return new PluginPermissionsResolver((PermissionsProvider) plugin, plugin);
		}
