...
public class bPermissionsResolver implements PermissionsResolver {
	...
	public boolean hasPermission(OfflinePlayer player, String permission) {
		...
		if (onlinePlayer == null) return ApiLayer.hasPermission(null, CalculableType.USER, player.getName(), permission);
		else return ApiLayer.hasPermission(onlinePlayer.getWorld().getName(), CalculableType.USER, player.getName(), permission);
	}
	...
	public boolean inGroup(OfflinePlayer player, String group) {
		Player onlinePlayer = player.getPlayer();
		if (onlinePlayer == null) return ApiLayer.hasGroupRecursive(null, CalculableType.USER, player.getName(), group);
		else return ApiLayer.hasGroupRecursive(onlinePlayer.getWorld().getName(), CalculableType.USER, player.getName(), group);
	}
	
	@Override
	public String[] getGroups(OfflinePlayer player) {
		Player onlinePlayer = player.getPlayer();
		if (onlinePlayer == null) return ApiLayer.getGroups(null, CalculableType.USER, player.getName());
		else return ApiLayer.getGroups(onlinePlayer.getWorld().getName(), CalculableType.USER, player.getName());
	}
}
