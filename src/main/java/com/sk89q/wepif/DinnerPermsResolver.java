...
public class DinnerPermsResolver implements PermissionsResolver {
	...
	public boolean hasPermission(OfflinePlayer player, String permission) {
		...
		if (perms == null) return false; // Permissions are only registered for objects with a
																			// Permissible
	...
	public boolean hasPermission(String worldName, OfflinePlayer player, String permission) {
		return hasPermission(player, permission); // no per-world ability to check permissions in
																							// dinnerperms
	}
	...
	public String[] getGroups(OfflinePlayer player) {
		Permissible perms = getPermissible(player);
		if (perms == null) return new String[0];
	 ...
	 * @return -1 if the permission is explicitly denied, 1 if the permission is allowed, 0 if the
	 *         permission is denied by a default. */
	public int internalHasPermission(Permissible perms, String permission) {
		if (perms.isPermissionSet(permission)) return perms.hasPermission(permission) ? 1 : -1;
		else {
			Permission perm = server.getPluginManager().getPermission(permission);
			if (perm != null) return perm.getDefault().getValue(perms.isOp()) ? 1 : 0;
			else return 0;
		}
	}
